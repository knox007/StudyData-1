...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		OFN.nMaxFile=10000;								//	OFN.lpstrFile의 길이.
														//	최소 256.

		OFN.Flags = OFN_ALLOWMULTISELECT				//	복수 개의 파일을 선택.
					| OFN_EXPLORER;						//	OFN_EXPLORER를 같이 사용하지 않으면 구형 대화상자가 열림.
					

		if (GetOpenFileName(&OFN)!=0) {
			p=lpstrFile;				//	2개이상의 파일은 선택한 경우
										//	폴더 경로\0파일명\0파일명.....\0\0
			lstrcpy(Name, p);
			...
			if (CommDlgExtendedError()==FNERR_BUFFERTOOSMALL)	//	공통 대화상자에서 에러 발생시 어떤 종류의 에러인지 확인.
