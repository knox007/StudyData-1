...
void McColor_TransColorGamma(DWORD* pOut, DWORD* pIn, INT iImgW, INT iImgH, D3DXCOLOR xGamma)
{
	for(int y=0; y<iImgH; ++y)
	{
		for(int x=0; x<iImgW; ++x)
		{
			...
			xclr.r = powf(xclr.r, 1.f/xGamma.r);
			xclr.g = powf(xclr.g, 1.f/xGamma.g);
			xclr.b = powf(xclr.b, 1.f/xGamma.b);
...
HRESULT CMain::Init()
{
	...
	McColor_TransColorGamma(pColorDst, pColorSrc, nImgW, nImgH, D3DXCOLOR(0.5F, 0.5F, 0.5F, 1.F));
