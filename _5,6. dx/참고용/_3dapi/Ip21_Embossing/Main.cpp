...
void McColor_TransColorMaskEmposing(DWORD* pOut, DWORD* pIn, INT iImgW, INT iImgH, FLOAT* fMask, INT iMskW)
{
	int y=0;
	int x=0;
	...
	DWORD ** pTemp= new DWORD*[iImgH];

	for(y=0; y<iImgH; ++y)
	{
		pTemp[y] = new DWORD[iImgW];
		...
		for(x=0; x<iImgW; ++x)
		{
			...
			fGray = xClr.r * 0.3f + xClr.g * 0.59f + xClr.b * 0.11f;
			fGrayAv+=fGray;

			pTemp[y][x] = D3DXCOLOR(fGray, fGray, fGray, 1.f);
		}
	}
	...
	for(y=0; y<iImgH; ++y)
	{
		for(int x=0; x<iImgW; ++x)
		{
			...
			for(int nMskY=nBgnY; nMskY<=nEndY; ++nMskY)
			{
				...
				{
					D3DXCOLOR xClr = pTemp[y+nMskY][x+nMskX];
...
HRESULT CMain::Init()
{
	...
	INT iMaskW = sizeof(g_MaskSouthEast[0])/(sizeof(g_MaskSouthEast[0][0]));
	McColor_TransColorMaskEmposing(pColorDst
									, pColorSrc, nImgW, nImgH
									, (float*)g_MaskSouthEast
									, iMaskW);
