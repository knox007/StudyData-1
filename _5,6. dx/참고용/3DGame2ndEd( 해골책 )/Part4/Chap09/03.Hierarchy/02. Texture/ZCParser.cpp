...
int	ZCParser::_ParseInfo( ZCParsedData* pData )
{
	...
	assert( g_pLog->Log( "Parsing Info" ) );

	try
	{
		pList = m_xml.FindElement( "Body/Info/ObjectCount" );	pList->get_item( 0, &pNode );	m_xml.GetElementText( pNode, str );
		pData->m_info.nObjectCount = atoi( str );

		pList = m_xml.FindElement( "Body/Info/MeshCount" );	pList->get_item( 0, &pNode );	m_xml.GetElementText( pNode, str );
		pData->m_info.nMeshCount = atoi( str );
		
		pList = m_xml.FindElement( "Body/Info/AnimationStart" );	pList->get_item( 0, &pNode );	m_xml.GetElementText( pNode, str );
		pData->m_info.fAnimationStart = atoi( str );
		
		pList = m_xml.FindElement( "Body/Info/AnimationEnd" );	pList->get_item( 0, &pNode );	m_xml.GetElementText( pNode, str );
		pData->m_info.fAnimationEnd = atoi( str );
		
		pList = m_xml.FindElement( "Body/Info/KeyType" );	pList->get_item( 0, &pNode );	m_xml.GetElementText( pNode, str );
		pData->m_info.nKeyType = 1; //atoi( str );	/// must be completed later.
		
		pList = m_xml.FindElement( "Body/Info/Object/Value" );
		pList->get_length( &nCount );
		pData->m_info.strObjectNameTable.resize( nCount );
		for( i = 0 ; i < nCount ; i++ )
		{
			pList->get_item( i, &pNode );
			m_xml.GetElementText( pNode, str );
			pData->m_info.strObjectNameTable[i] = str;
			assert( g_pLog->Log( "Objects[%d] = %s", i, pData->m_info.strObjectNameTable[i].c_str() ) );
		}

		pList = m_xml.FindElement( "Body/Info/Bone/Value" );
		pList->get_length( &nCount );
		...
		for( i = 0 ; i < nCount ; i++ )
		{
			pList->get_item( i, &pNode );
			m_xml.GetElementText( pNode, str );
			m_xml.GetAttributeText( pNode, "ID", str );
			pData->m_info.BoneTable[i] = atoi( str );
			assert( g_pLog->Log( "Bone[%d] = %s", i, pData->m_info.strObjectNameTable[atoi(str)].c_str() ) );
		}
...
int	ZCParser::_ParseMaterial( ZCParsedData* pData )
{
	...
	assert( g_pLog->Log( "Parsing Material" ) );
	...
	try
	{
		...
		{
			pList->get_item( i, &pNode );
			pListChild = m_xml.FindElement( pNode, "Diffuse" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].diffuse = _StrToColor3f( str );

			pListChild = m_xml.FindElement( pNode, "Ambient" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].ambient = _StrToColor3f( str );

			pListChild = m_xml.FindElement( pNode, "Specular" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].specular = _StrToColor3f( str );

			pListChild = m_xml.FindElement( pNode, "Emissive" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].emissive = _StrToColor3f( str );
			
			pListChild = m_xml.FindElement( pNode, "Opacity" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].opacity = atof( str );

			pListChild = m_xml.FindElement( pNode, "Power" );
			pListChild->get_item( 0, &pNodeChild );
			m_xml.GetElementText( pNodeChild, str );
			pData->m_materialTable[i].power = atof( str );

			pListChild = m_xml.FindElement( pNode, "DiffuseMap" );
			...
			pListChild = m_xml.FindElement( pNode, "SpecularMap" );
			if( S_OK == pListChild->get_item( 0, &pNodeChild ) )
			{
				m_xml.GetElementText( pNodeChild, str );
			...
			pListChild = m_xml.FindElement( pNode, "BumpMap" );
			if( S_OK == pListChild->get_item( 0, &pNodeChild ) )
			{
				m_xml.GetElementText( pNodeChild, str );
			...
			pListChild = m_xml.FindElement( pNode, "DisplacementMap" );
			if( S_OK == pListChild->get_item( 0, &pNodeChild ) )
			{
				m_xml.GetElementText( pNodeChild, str );
...
int	ZCParser::_ParseObject( ZCParsedData* pData )
{
	...
	assert( g_pLog->Log( "Parsing Objects..." ) );

	try
	{
		pList = m_xml.FindElement( "Body/Object" );
		pList->get_length( &nCount );
		for( i = 0 ; i < nCount ; i++ )
		{
			pMesh = new ZCMesh();
			...
			m_xml.GetAttributeText( pNode, "Class", str );
			pMesh->m_strNodeType = str;

//			assert( g_pLog->Log( " " ) );
//			assert( g_pLog->Log( "Parsing [%s]", pData->m_info.strObjectNameTable[ pMesh->m_nObjectID ].c_str() ) );
...
int	ZCParser::_ParseObjectParent( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "Parent" );
		pListChild->get_item( 0, &pNodeChild );
		m_xml.GetAttributeText( pNodeChild, "ID", str );
...
int	ZCParser::_ParseObjectBBox( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "BoundingBox/Max" );
		pListChild->get_item( 0, &pNodeChild );
		m_xml.GetElementText( pNodeChild, str );
		pMesh->m_bboxMax = _StrToVector3f( str );

		pListChild = m_xml.FindElement( pNode, "BoundingBox/Min" );
		pListChild->get_item( 0, &pNodeChild );
		m_xml.GetElementText( pNodeChild, str );
...
int	ZCParser::_ParseObjectTM( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "LocalTM/Value" );
...
int	ZCParser::_ParseObjectVertex( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "Vertex/Value" );
		pListChild->get_length( &nCount );
...
int	ZCParser::_ParseObjectVertexNormal( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "VertexNormal/Value" );
		pListChild->get_length( &nCount );
...
int	ZCParser::_ParseObjectTVertex( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "TexVertex/Value" );
		pListChild->get_length( &nCount );
...
int	ZCParser::_ParseObjectTriIndex( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	{
		/// Get material count
		pListChild = m_xml.FindElement( pNode, "TriIndex" );
		pListChild->get_item( 0, &pNodeChild );
		if( pNodeChild == NULL )
		{
		...
		m_xml.GetAttributeText( pNodeChild, "MaterialCount", str );
		pMesh->m_nMaterialCount = atoi( str );

		pListChild = m_xml.FindElement( pNode, "TriIndex/Value" );
		pListChild->get_length( &nCount );
		...
		{
			pListChild->get_item( i, &pNodeChild );
			m_xml.GetAttributeText( pNodeChild, "MaterialID", str );
...
int	ZCParser::_ParseObjectTexIndex( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "TexIndex/Value" );
		pListChild->get_length( &nCount );
...
int	ZCParser::_ParseObjectVertWeight( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "VertexWeight/Vertex" );
		pListChild->get_length( &nCount );
...
//				assert( g_pLog->Log( "vertex[%d] Bone ID=%d", i, pMesh->m_vertweight[i].ID[j] ) );
				...
				m_xml.GetElementText( pNodeChild2, str );
...
int	ZCParser::_ParseObjectKey( ZCMesh* pMesh, IXMLDOMNode* pNode )
{
	...
	try
	{
		pListChild = m_xml.FindElement( pNode, "Key/Position/Value" );
		pListChild->get_length( &nCount );
		pMesh->m_track.pos.resize( nCount );
		for( i = 0 ; i < nCount ; i++ )
		{
			pListChild->get_item( i, &pNodeChild );
			m_xml.GetAttributeText( pNodeChild, "Frame", str );
...
//			assert( g_pLog->Log( "[%d]th [%f]frame key=%s", i, pMesh->m_track.pos[i].frame, str ) );
		...
		pListChild = m_xml.FindElement( pNode, "Key/Quaternion/Value" );
		pListChild->get_length( &nCount );
...
//			assert( g_pLog->Log( "[%d]th [%f]frame key=%s", i, pMesh->m_track.rot[i].frame, str ) );
		...
		pListChild = m_xml.FindElement( pNode, "Key/Scale/Value" );
		pListChild->get_length( &nCount );
