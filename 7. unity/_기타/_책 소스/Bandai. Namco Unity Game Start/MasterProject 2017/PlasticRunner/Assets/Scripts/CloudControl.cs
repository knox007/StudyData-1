...
public class CloudControl : MonoBehaviour {
	...
	void	Start()
	{
		...
		this.face = this.transform.Find("face").gameObject;
		this.face_center = this.face.transform.localPosition.z;

		// 입.

		Transform	mouth_root = this.face.transform.Find("mouth");

		for(int i = 0;i < (int)MOUTH_TYPE.NUM;i++) {

			this.mouths.Add(null);

			Transform	t = mouth_root.Find("mouth" + i);
		...
		for(int i = 0;i < (int)EYE_TYPE.NUM;i++) {
			...
			Transform	t = this.face.transform.Find("eye" + i);
