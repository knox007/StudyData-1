...
ZFLog::ZFLog( UINT32 nTarget, LPSTR szFilename )
{
	...
	if( nTarget & ZF_LOG_TARGET_CONSOLE )
	{
		m_hwnd = NULL;
		FreeConsole();		// 기존의 콘솔을 지운다
		AllocConsole();		// 새로운 콘솔을 만든다
		SetConsoleTitle( "Flogger Console" );
	}

	Log( "Flogger Logging Started..." );
}
...
int ZFLog::Log( LPSTR fmt, ... )
{
	...
	if( m_nTarget & ZF_LOG_TARGET_CONSOLE )
	{
		...
		wsprintf( str, "(date[%s] time[%s]) : %s\n", date, time, buff );
		WriteConsole( h, str, lstrlen( str ), NULL, NULL );
	}
	...
	if( m_nTarget & ZF_LOG_TARGET_WINDOW )
	{
		...
		UINT32 n = (UINT32)SendMessage( m_hwndList, LB_GETCOUNT, 0, 0L ) - 1;
