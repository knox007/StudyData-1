...
int ZFFSkinnedMesh::_CreateVIB( ZCMesh* pMesh )
{
	...
	// 정점 버퍼 생성
	m_pDev->CreateVertexBuffer( m_nVerts * sizeof(ZTSkinnedVertex), 0, m_dwFVF, D3DPOOL_DEFAULT, &m_pVB, NULL );
	m_pVB->Lock( 0, m_nVerts * sizeof(ZTSkinnedVertex), (void**)&pV, 0 );
	memcpy( pV, &pMesh->m_vtxFinal[0], m_nVerts * sizeof(ZTSkinnedVertex) );
	m_pVB->Unlock();
...
int ZFFSkinnedMesh::Draw( D3DXMATRIX* pTM )
{
	...
	m_pDev->SetStreamSource( 0, m_pVB, 0, sizeof(ZTSkinnedVertex) );
