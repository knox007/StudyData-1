...
//-----------------------------------------------------------------------
void	CPRXFileEntity::InitMembers()
{
	m_pmatBone		=	NULL;
	m_pFirstMesh	=	NULL;
	m_pFrameRoot 	=	NULL;
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::ClearMembers()
{
	...
	SAFE_DELETE_ARRAY( m_pmatBone );

	SAFE_DELETE( m_pXMeshHierachy );

}//	ClearMembers
//-------------------------------------------------------------------------------
bool	CPRXFileEntity::Init( PRLPD3DDEV9 pDev, char *pszXFile, D3DXVECTOR3 vPos, D3DXVECTOR3 vScale, bool bRepeatAni )
{
...
//-------------------------------------------------------------------------------
bool	CPRXFileEntity::Load( PRLPD3DDEV9 pDev, const char *pszFName )
{
...
//	SAFE_DELETE( pMemAlloc );
	...
	{
		//	본 설정.
		SetupBoneMatrices( pDev, (sPRD3DXFRAMEEx*)GetRootFrame(), NULL );
...
//-------------------------------------------------------------------------------
D3DXVECTOR3	CPRXFileEntity::GetInitCameraPos() const
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetupBoneMatrices( PRLPD3DDEV9 pDev, sPRD3DXFRAMEEx *pFrame, LPD3DXMATRIX pmatParent )
{
	//	먼저 확장된 구조체로 캐스트한다.
	sPRD3DXMESHCONTAINEREx	*pMesh	=	(sPRD3DXMESHCONTAINEREx*)pFrame->pMeshContainer;
	...
	if( pMesh )
	{
		...
		if( pMesh->pSkinInfo && pMesh->MeshData.pMesh )
		{
			...
			PRDVTXELE9	d3dDclr[ MAX_FVF_DECL_SIZE ]	=	{0,};
			...
			{
				//	본을 가지고 있는 프레임을 찾는다.
				sPRD3DXFRAMEEx	*pTmpFrame	=	(sPRD3DXFRAMEEx*)D3DXFrameFind( GetRootFrame(), pMesh->pSkinInfo->GetBoneName( unCur ) );
	...
	if( pFrame->pFrameSibling )
		SetupBoneMatrices( pDev, (sPRD3DXFRAMEEx*)pFrame->pFrameSibling, pmatParent );

	//	자식 프레임으로 넘어간다.
	if( pFrame->pFrameFirstChild )
		SetupBoneMatrices( pDev, (sPRD3DXFRAMEEx*)pFrame->pFrameFirstChild, &pFrame->_matCombinedTransformEx );

}//	SetupBoneMatrices
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetRot( D3DXVECTOR3 vecRot )
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::UpdateFrameMatrices( const LPD3DXFRAME pFrameBase, const D3DXMATRIX *pmatParent )
{
	sPRD3DXFRAMEEx	*pCurFrame	=	(sPRD3DXFRAMEEx*)pFrameBase;
...
//-------------------------------------------------------------------------------
HRESULT	CPRXFileEntity::OnRender( PRLPD3DDEV9 pDev, float fElapsedTime )
{	
	if( GetRootFrame() )
	{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::DrawFrame( PRLPD3DDEV9 pDev, LPD3DXFRAME pFrame ) const
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::DrawMeshContainer( PRLPD3DDEV9 pDev, LPD3DXMESHCONTAINER pMeshContainerBase, LPD3DXFRAME pFrameBase ) const
{
	//	확장 프레임 타입으로 캐스팅
	sPRD3DXFRAMEEx	*pFrame	=	( sPRD3DXFRAMEEx* )pFrameBase;

	//	확장 메쉬 컨테이너로 캐스팅
	sPRD3DXMESHCONTAINEREx *pMeshContainer	=	(sPRD3DXMESHCONTAINEREx*)pMeshContainerBase;
...
//	devSetTSSCOLORARG1( pDev, 0, D3DTA_TEXTURE );
	...
	for( UINT unCur = 0; unCur < pMeshContainer->NumMaterials; ++unCur )
	{
		...
		devSetTxtz( pDev, 0, pMeshContainer->_ppTexturesEx[ unCur ] );

		LPD3DXMESH	pDrawMesh	=	( pMeshContainer->pSkinInfo ) ? pMeshContainer->_pSkinMeshEx : pMeshContainer->MeshData.pMesh;
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetAniSet( UINT unIdx )
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetAniNext()
{
...
//-------------------------------------------------------------------------------
std::string	CPRXFileEntity::GetAniSetName( UINT unIdx )
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetAniSlower()
{
	m_fSpeedAdjust	+=	0.1f;

}//	SetAniSlower
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetAniFaster()
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::SetAniStart()
{
...
//-------------------------------------------------------------------------------
void	CPRXFileEntity::OnUpdate( float fElapsed )
{
	D3DXMATRIX	matScale;
	D3DXMATRIX	matPos;
	D3DXMATRIX	matRot;
	...
	sPRD3DXMESHCONTAINEREx	*pMesh	=	m_pFirstMesh;
