...
{
	ZTBOXVERTEX vtx[8];
	vtx[0] = ZTBOXVERTEX( m_bboxMinOrig.x, m_bboxMaxOrig.y, m_bboxMaxOrig.z, 0xffff0000 );
	vtx[1] = ZTBOXVERTEX( m_bboxMaxOrig.x, m_bboxMaxOrig.y, m_bboxMaxOrig.z, 0xffff0000 );
	vtx[2] = ZTBOXVERTEX( m_bboxMaxOrig.x, m_bboxMaxOrig.y, m_bboxMinOrig.z, 0xffff0000 );
	vtx[3] = ZTBOXVERTEX( m_bboxMinOrig.x, m_bboxMaxOrig.y, m_bboxMinOrig.z, 0xffff0000 );
	vtx[4] = ZTBOXVERTEX( m_bboxMinOrig.x, m_bboxMinOrig.y, m_bboxMaxOrig.z, 0xffff0000 );
	vtx[5] = ZTBOXVERTEX( m_bboxMaxOrig.x, m_bboxMinOrig.y, m_bboxMaxOrig.z, 0xffff0000 );
	vtx[6] = ZTBOXVERTEX( m_bboxMaxOrig.x, m_bboxMinOrig.y, m_bboxMinOrig.z, 0xffff0000 );
	vtx[7] = ZTBOXVERTEX( m_bboxMinOrig.x, m_bboxMinOrig.y, m_bboxMinOrig.z, 0xffff0000 );
	...
	m_pDev->SetFVF( ZTBOXVERTEX::FVF );
	m_pDev->DrawIndexedPrimitiveUP( D3DPT_TRIANGLELIST, 0, 8, 12, idx, D3DFMT_INDEX16, vtx, sizeof ZTBOXVERTEX );
