...
int ZSWSkinnedMesh::_CreateVIB( ZCMesh* pMesh )
{
	...
	m_pvtx = new ZTSWSkinnedVertex[m_nVerts];
	m_pvtxDraw = new ZTRigidVertex[m_nVerts];
...
int ZSWSkinnedMesh::Draw( D3DXMATRIX* pTM )
{
	...
	m_pDev->DrawIndexedPrimitiveUP( D3DPT_TRIANGLELIST, 0, m_nVerts, m_nTriangles, m_pidx, m_fmtIdx, m_pvtxDraw, sizeof(ZTRigidVertex) );
	return 1;
}
