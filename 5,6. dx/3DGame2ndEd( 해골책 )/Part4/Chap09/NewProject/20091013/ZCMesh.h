...
public:
	...
	std::vector<Vector3f>	m_vertex;				/// 정점의 위치정보
	std::vector<Vector3f>	m_normal;				/// 정점의 노멀정보
	std::vector<Vector2f>	m_tvertex;				/// 텍스쳐 정보

	int						m_nMaterialID;			/// 재질의 ID
	int						m_nMaterialCount;		/// 메시에서 사용중인 재질의 개수
	std::vector<IndexWithID>	m_triindex;				/// 삼각형의 인덱스
	std::vector<Index3i>	m_texindex;				/// 텍스쳐의 인덱스
	std::vector<VertWeight>	m_vertweight;			/// 정점의 가중치값
	Track					m_track;				/// 애니메이션 트랙정보
public:
	std::vector<VertMerged>	m_vtxFinal;				/// 정점관련 정보들을 종합한 최종 정점
