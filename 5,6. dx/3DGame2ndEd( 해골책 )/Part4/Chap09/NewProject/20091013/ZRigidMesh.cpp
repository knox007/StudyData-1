...
int ZRigidMesh::_CreateVIB( ZCMesh* pMesh )
{
	...
	std::vector<ZTRigidVertex>	vtx;

	m_nVerts = pMesh->m_vtxFinal.size();;
	m_dwFVF = ZTRigidVertex::FVF;
	...
	// 정점 버퍼 생성
	m_pDev->CreateVertexBuffer( m_nVerts * sizeof(ZTRigidVertex), 0, m_dwFVF, D3DPOOL_DEFAULT, &m_pVB, NULL );
	m_pVB->Lock( 0, m_nVerts * sizeof(ZTRigidVertex), (void**)&pV, 0 );
	memcpy( pV, &vtx[0], m_nVerts * sizeof(ZTRigidVertex) );
	m_pVB->Unlock();
	stl_wipe_vector( vtx );
...
int ZRigidMesh::Draw( D3DXMATRIX* pTM )
{
	...
	m_pDev->SetStreamSource( 0, m_pVB, 0, sizeof(ZTRigidVertex) );
