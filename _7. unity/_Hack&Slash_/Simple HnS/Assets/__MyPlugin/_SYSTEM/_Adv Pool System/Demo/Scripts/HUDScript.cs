...
public class HUDScript : MonoBehaviour {
	...
	void Update () {
		...
		for(int i=0; i<pS._poolingUnits.Length; i++)
		{
			count[i] = pS._pooledUnitsList[i].Count;
		}

		count[3]=0;
		for(int j=0; j<pS._pooledUnitsList[0].Count; j++)
		{
			if(pS._pooledUnitsList[0][j].activeInHierarchy)
			{
				count[3]++;
			}
		}

		pS._canPoolExpand = expandable;
