...
INT CInputManager::FrameMove()	// Mouse
{
	...
	for(curIdx = 0; curIdx < MAX_INPUT_BTN; ++curIdx)
	{
		...
		if		(0 == nOld && 1 ==nCur) m_mouseBtttnMap[curIdx] = DOWN;	 // Down  (키 다운)
		else if (1 == nOld && 0 ==nCur) m_mouseBtttnMap[curIdx] = UP;	 // UP	  (키 업)
		else if (1 == nOld && 1 ==nCur) m_mouseBtttnMap[curIdx] = PRESS; // PRESS (키 다운 유지)
